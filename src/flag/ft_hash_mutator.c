...
/*   Updated: 2023/11/11 06:44:34 by kiroussa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static char	*ft_hex_prefix(char specifier)
{
	if (specifier == 'o')
		return ("0");
	else if (specifier == 'x')
...
char	*ft_hash_mutator_prefix(t_fmt_spec *spec, char *str)
{
	...
	len = ft_strlen(str);
	if (spec->flags & PF_HASH && ft_hex_prefix(spec->specifier))
	{
		if (*str == '0' && ft_strlen(str) == 1)
			return (str);
		prefix = ft_hex_prefix(spec->specifier);
